...
function GroupRow({ group, onView, index, selected, onToggle }) {
  ...
  return (
    <div className={`bg-[var(--bg-surface)] border-2 rounded-2xl overflow-hidden transition-all duration-150
      ${selected
        ...
        : 'border-[var(--border)] hover:border-indigo-400/40 dark:hover:border-indigo-500/30 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/5 dark:hover:shadow-black/20'}`}>
...
export default function AdminGroups() {
  ...
  const selectAllDept = (dept) => {
    const deptIds = filtered.filter(g => g.department === dept).map(g => g.id)
    const allSelected = deptIds.every(id => selected.has(id))
    setSelected(prev => {
      const n = new Set(prev)
      if (allSelected) deptIds.forEach(id => n.delete(id))
      else deptIds.forEach(id => n.add(id))
      return n
    })
  }
  ...
  const downloadPDF = (dept) => {
    const selectedGroups = groups.filter(g => selected.has(g.id) && (dept ? g.department === dept : true))
    if (!selectedGroups.length) return alert(`No ${dept||''} groups selected.`)
    generateDeptPDF(selectedGroups, dept)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">Select groups below then download PDF by department</p>
        </div>
      ...
      {/* Selection action bar — shows when something selected */}
      {selectedCount > 0 && (
        <div className="flex items-center justify-between gap-4 px-5 py-3.5 bg-indigo-50 dark:bg-indigo-500/10 border-2 border-indigo-200 dark:border-indigo-500/30 rounded-2xl flex-wrap">
          <div className="flex items-center gap-2">
            ...
            {seSelected>0 && <Badge variant="brand">{seSelected} SE</Badge>}
            {csSelected>0 && <Badge variant="cs">{csSelected} CS</Badge>}
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            {seSelected > 0 && (
              <Button variant="secondary" size="sm" onClick={() => downloadPDF('SE')}>
                <Icon name="fileText" size={13}/> Download SE PDF
              </Button>
            )}
            {csSelected > 0 && (
              <Button variant="secondary" size="sm" onClick={() => downloadPDF('CS')}>
                <Icon name="fileText" size={13}/> Download CS PDF
              </Button>
            )}
            {selectedCount > 1 && seSelected > 0 && csSelected > 0 && (
              <Button variant="primary" size="sm" onClick={() => downloadPDF(null)}>
                <Icon name="fileText" size={13}/> Download All PDF
              </Button>
            )}
            <button onClick={() => setSelected(new Set())}
              className="text-xs text-indigo-600 dark:text-indigo-400 font-semibold hover:underline ml-1">
              Clear
            </button>
          </div>
        </div>
      ...
      {!loading && filtered.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {['SE','CS'].map(d => {
            const deptGroups = filtered.filter(g=>g.department===d)
            if (!deptGroups.length) return null
            const allSel = deptGroups.every(g=>selected.has(g.id))
            return (
              <button key={d} onClick={()=>selectAllDept(d)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all
                  ${d==='SE'
                    ? 'border-orange-200 dark:border-orange-400/25 text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-400/8 hover:bg-orange-100 dark:hover:bg-orange-400/15'
                    : 'border-cyan-200 dark:border-cyan-400/25 text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-cyan-400/8 hover:bg-cyan-100 dark:hover:bg-cyan-400/15'}`}>
                {allSel ? `Deselect ${d}` : `Select All ${d}`} ({deptGroups.length})
              </button>
            )
          })}
        </div>
